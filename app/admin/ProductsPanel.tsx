...
// Çoklu dili string'e çeviren güvenli yardımcı fonksiyon
function getLangText(val: MultiLangString | undefined, lang: "tr" | "en" = "tr"): string {
  if (!val) return "";
  if (typeof val === "string") return val; // Eski düz string kayıtlar için
...
export default function ProductsPanel() {
  ...
  async function handleDeleteProduct(product: Product) {
    ...
    if (!confirm(`"${productName}" ürünü kalıcı olarak silinsin mi?`)) return;
  ...
  function categoryName(id: string | null) {
    ...
    if (!cat) return "Kategorisiz";
    return getLangText(cat.name); // Sadece Türkçesi görünsün
  }
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {products.map((product) => {
            const prodName = getLangText(product.name);
            const prodSubtitle = getLangText(product.subtitle);
              ...
              <div key={product.id} className="group relative flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white">
                ...
                <div className="flex flex-1 flex-col justify-between p-5">
                  <div>
                    ...
                    <h3 className="mb-1 text-base font-extrabold text-[#3A3A3A]">{prodName}</h3>
                    {prodSubtitle && <p className="line-clamp-2 text-xs text-gray-500">{prodSubtitle}</p>}
                  </div>
                </div>
              </div>
            );
